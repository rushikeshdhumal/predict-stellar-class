...
SCRIPT_PATH = Path(OUT_DIR) / "ft_transformer_ddp_train.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-name", type=str, default="ft_transformer_ddp")
...
def train_fold(
    ...
    criterion = nn.CrossEntropyLoss(weight=class_weight_tensor(y_train).cuda(local_rank))
    ...
    for epoch in range(args.epochs):
        ...
        if rank == 0:
            ...
            print(f"{args.model_name} fold {fold} epoch {epoch + 1}: balanced_accuracy={score:.6f}", flush=True)
        dist.barrier()
    ...
    torch.save({"model_state_dict": model.module.state_dict(), "encoding": encoding, "args": vars(args)}, model_dir / f"{args.model_name}_fold_{fold}.pt")
...
def predict_proba(model: FTTransformer, numeric: np.ndarray, categorical: np.ndarray, batch_size: int, local_rank: int) -> np.ndarray:
    model.eval()
    probabilities = []
    loader = DataLoader(
...
def class_weight_tensor(y_train: np.ndarray) -> torch.Tensor:
    counts = np.bincount(y_train, minlength=len(CLASS_LABELS)).astype(np.float32)
